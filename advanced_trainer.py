...
class IncrementalTrainer:
    ...
    def incremental_update(self):
        ...
        # Ensure y contains all classes to prevent XGBoost ValueError for missing classes
        all_class_indices = set(range(len(le.classes_)))
        present_class_indices = set(y)
        missing_class_indices = list(all_class_indices - present_class_indices)
        
        sample_weight = np.ones(len(y))
        
        if missing_class_indices:
            log.info(f"Padding training batch with {len(missing_class_indices)} dummy samples for missing classes")
            dummy_X = np.zeros((len(missing_class_indices), X_scaled.shape[1]))
            X_scaled = np.vstack([X_scaled, dummy_X])
            y = np.concatenate([y, missing_class_indices])
            sample_weight = np.concatenate([sample_weight, np.zeros(len(missing_class_indices))])
        ...
        try:
            ...
            new_model.fit(X_scaled, y, xgb_model=model.get_booster(), sample_weight=sample_weight)
        except Exception as e:
            log.warning(f"Warm-start failed ({e}), falling back to fresh fit")
            new_model = xgb.XGBClassifier(
                n_estimators=100, learning_rate=0.1, max_depth=6
            )
            new_model.fit(X_scaled, y, sample_weight=sample_weight)
